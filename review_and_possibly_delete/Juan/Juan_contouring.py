...
def clear_folder_contents(folder_path):
    ...
    log(f"Total files deleted from {folder_path}: {files_deleted}")
    return files_deleted


# Function to Deletes all feature classes within the specified feature dataset
...
def main():
    ...
    log(f"Deleting contour_lines_sp_feature_class: {contour_lines_sp_feature_class}")
    check_and_delete(contour_lines_sp_feature_class)
    log(f"Deleting contour_lines_sp in the wrong location, if any: {contour_lines_sp_wrong}")
    check_and_delete(contour_lines_sp_wrong)
    
    # Ensure there is no Boundary_UTM from an old model
    log(f"Deleting Boundary UTM from old models, if any: {boundary_utm}")
    ...
    try:
        ...
        log(f"Recalculation of Feature Class Extent completed. Output: {contour_lines_sp_feature_class}")
        ...
        log(f"Repairing of Geometry completed. Output: {contour_lines_sp_feature_class}")
        ...
        arcpy.management.AddField(
            in_table=contour_lines_sp_feature_class,
        ...
        for FC_Variable, Name in FeatureClassGenerator(output_feature_dataset, "", "LINE", "NOT_RECURSIVE"):
            ...
            arcpy.analysis.Select(
                ...
                out_feature_class=_Name_2Ft_shp,
        ...
        # ################# Change made 9/10/2025 : “nodata” to -999999
        md_nodata = arcpy.management.DefineMosaicDatasetNoData(
            input_mosaic_dataset, 
            num_bands=1, 
            bands_for_nodata_value=[["ALL_BANDS", "-999999"]]
        )[0]
