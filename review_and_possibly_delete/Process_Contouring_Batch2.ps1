...
Just TEMP for last steps of shapefiles and DWGs for Alabama
...
$logFile = "Process_Contouring_Batch2.log"
...
function Start-NewTask {
    ...
    $pythonCommand = "cd '$pythonDir'; .\$pythonPath Z:\scripts\contouring2.py $state $county"
...
while ($remainingTasks -gt 0 -or $processes.Count -gt 0) {
    ...
    for ($i = 0; $i -lt $processes.Count; $i++) {
        ...
        if ($process.HasExited) {
            # Log task completion
            $completedTask = $process.StartInfo.Arguments -replace '.*contouring2\.py ', ''
    ...
    while ($processes.Count -lt 20 -and $remainingTasks -gt 0) {
