...
Syntax Example:
		cd D:\scripts\
		.\Process_ExportShapefilesToCAD_Batch.ps1 .\ALABAMA.csv
		
Notes:
		The script will open 4 minimized windows doing the Process_ExportShapefilesToCAD process for counties.
...
3. The script spawns new PowerShell processes to execute a Python script (`Process_ExportShapefilesToCAD.py` in this case) for each task, passing the state and county as parameters to the Python script.
4. Tasks are processed concurrently with a limit of 4 active processes at a time.
5. Active and completed tasks are logged in a file named 'Process_ExportShapefilesToCAD.log'.
...
- Log entries include timestamps and task information for both starting and completing tasks.
	Log stored at Process_ExportShapefilesToCAD.log
...
$logFile = "Process_ExportShapefilesToCAD.log"
...
function Start-NewTask {
    ...
    $pythonCommand = "cd '$pythonDir'; .\$pythonPath D:\scripts\ExportShapefilesToCAD.py $state $county"
...
while ($remainingTasks -gt 0 -or $processes.Count -gt 0) {
    ...
    for ($i = 0; $i -lt $processes.Count; $i++) {
        ...
        if ($process.HasExited) {
            # Log task completion
            $completedTask = $process.StartInfo.Arguments -replace '.*ExportShapefilesToCAD\.py ', ''
    ...
    while ($processes.Count -lt 1 -and $remainingTasks -gt 0) {
