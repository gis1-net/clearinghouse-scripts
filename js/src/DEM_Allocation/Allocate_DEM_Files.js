...
const RECENT_AUTOMATIC_USE_MINIMUM_COVERAGE = 0.03
...
const main = (index, silent = false) => {
  ...
  const data = readJsonData(`Lidar_Project_Boundaries_By_County/${county.properties.STATE}/${county.properties.NAME}_${county.properties.LSAD}_Lidar_Project_Boundaries.geojson`).features
  ...
  for (let key in countyProjects) {
    ...
    console.log(county.properties.NAME, key, coverage)
  ...
  while (useProjects.length > 0) {
    ...
    if (!silent) {
      console.log(`${project.properties.workunit} County Coverage: ${coverage}`)
    }
