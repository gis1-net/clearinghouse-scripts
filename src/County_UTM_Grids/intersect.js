...
const main = async () => {
  ...
  // for (let i in counties.features) {
    ...
    if (!fs.existsSync(`${__dirname}/../../data/County_UTM_Grids/${county.properties.STATE}`)) {
      fs.mkdirSync(`${__dirname}/../../data/County_UTM_Grids/${county.properties.STATE}`)
    }

    console.log(`Writing ${county.properties.STATE}/${county.properties.NAME}.json`)
    fs.writeFileSync(`${__dirname}/../../data/County_UTM_Grids/${county.properties.STATE}/${county.properties.NAME}.json`, JSON.stringify({ type: 'FeatureCollection', features: tiles }, null, 2))
  // }
}
